...
產出 `out/selects.html`（單一自足檔）。每支影片一節：**全幅可縮放時間軸**
（ctrl+滾輪縮放、shift+滾輪平移、點一下跳播）＋左側影片與圈選面板＋右側清單獨立捲動。

- **語音段（上排、綠色）**：每句 ASR 一個候選段——點跳播、勾 checkbox 匯出、
  清單裡改起訖秒數微調、改備註。
- **手動段（下排、藍色）**：在時間軸上**直接拖曳圈一段**→左側面板顯示起訖與
  長度→微調數值、寫備註、可「試聽」→按**「加入」**進輸出清單。圈任何畫面
  （b-roll、操作畫面、無語音段落）。
- **切割**：播放頭停在段內按 `s`，該段一分為二，兩半各自勾選。
- 鍵盤流：`space` 播放、`←→` ±0.5s、`s` 切割、`e` 勾/取消、`↑↓` 換段、`✕` 刪段。
